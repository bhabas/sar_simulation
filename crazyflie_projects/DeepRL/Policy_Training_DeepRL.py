...
class Policy_Trainer_DeepRL():
    ...
    def test_landing_performance(self,fileName=None,Vel_inc=0.25,Phi_inc=5,n_episodes=20):
        ...
        if fileName == None:
            fileName = "PolicyPerformance_Data_20Trials_Noisex20.csv"
    ...
    ## CONFIGURATION COMPARISON w/Noise
    def Configuration_Comparison_Noisy(self,fileName=None,saveFig=True):
        avg = []
        avgBC4 = []
        avgLeg2 = []
        avgNoLeg = []
        ...
        for filename in sorted(os.listdir(self.log_dir)):
            f = os.path.join(self.log_dir, filename, self.log_name)
            d = os.path.join(self.log_dir, filename)
            check_file = os.path.isfile(f)
            if filename == "PolicyPerformance_Data_20Trials_Noisex20.csv_0":
                os.rmdir(os.path.join(self.log_dir, filename, "models"))
                os.rmdir(os.path.join(self.log_dir, filename)) 
            else:
                if check_file:
                    ...
                    af = pd.read_csv(f,sep=',',comment="#")
                    afnotnoise = pd.read_csv(f'{os.path.join(self.log_dir, filename)}/PolicyPerformance_Data_20Trials.csv',sep=',',comment="#")
                    af2 = af.groupby(['Vel_d','Phi_d']).mean().round(3).reset_index()
                    afnotnoise2 = afnotnoise.groupby(['Vel_d','Phi_d']).mean().round(3).reset_index()



                    ## COLLECT DATA
                    R = afnotnoise2.iloc[:]['Vel_d']
                    Theta = afnotnoise2.iloc[:]['Phi_d']
                    C = afnotnoise2.iloc[:]['4_Leg_NBC']
                    Rnoise = af2.iloc[:]['Vel_d']
                    Thetanoise = af2.iloc[:]['Phi_d']
                    Cnoise = af2.iloc[:]['4_Leg_NBC']
                    # C1 = af2.iloc[:]['2_Leg_NBC']
                    # C2 = af2.iloc[:]['2_Leg_BC']
                    # C = np.add(C1,C2)
                    ...
                    def opt(thresh, data):
                        ...
                        for i in range(0,len(data)):
                            if data[i]<thresh:
                                blthresh.append(np.sqrt(thresh-data[i]))
                            else:
                                abthresh.append(np.square(data[i]-thresh))
                        optim = np.mean(abthresh)-np.mean(blthresh)
                        print(optim)
                        return optim
                    ...
                    #     gr = np.divide((yvar(xvar[i+1], C)-yvar(xvar[i], C)), (xvar[i+1]-xvar[i]))
                    #     return gr

                    
                    # def gradient_descent(start, learn_rate, max_iter, tol=0.01):
                    ...
                    threshopt = []
                    for i in range(0,46):
                        t = np.divide((50+i), 100)
                        optimum = opt(t, C)
                        threshopt.append(optimum)
                        
                    thresh = 0.5+np.divide((threshopt.index(min(threshopt))),100)
                    print(thresh) 
                    thresh = 0.85  
                    ...
                    for i in range(0, 169):
                        point = C[168-i]
                        if C[168-i]>0:
                            robust = np.divide(Cnoise[168-i], C[168-i])
                        else:
                            robust = 1
                        if point<thresh:
                            C[168-i] = 0
                    ...
                    ## DEFINE INTERPOLATION GRID
                    Rnoise_list = np.linspace(Rnoise.min(),Rnoise.max(),num=50,endpoint=True).reshape(1,-1)
                    Thetanoise_list = np.linspace(Thetanoise.min(),Thetanoise.max(),num=50,endpoint=True).reshape(1,-1)
                    Rnoise_grid, Thetanoise_grid = np.meshgrid(Rnoise_list, Thetanoise_list)
                    
                    ## INTERPOLATE DATA
                    LR_interp = griddata((R, Theta), C, (R_list, Theta_list.T), method='nearest')
                    LR_interp += 0.001

                    ## INTERPOLATE DATA
                    LRnoise_interp = griddata((Rnoise, Thetanoise), Cnoise, (Rnoise_list, Thetanoise_list.T), method='nearest')
                    LRnoise_interp += 0.001
                    
                    # get colormap
                    ncolors = 256
                    color_array = plt.get_cmap('gist_yarg')(range(ncolors))

                    # change alpha values
                    color_array[:,-1] = np.linspace(0.0,1.0,ncolors)

                    # create a colormap object
                    map_object = LinearSegmentedColormap.from_list(name='yarg_alpha',colors=color_array)
                    ...
                    fig = plt.figure(figsize=(6,6))
                    ax = fig.add_subplot(projection='polar')
                    cmap = map_object #mpl.cm.jet#gist_yarg
                    norm = mpl.colors.Normalize(vmin=0,vmax=1)
                    ...
                    cmapnoise = map_object #mpl.cm.jet
                    normnoise = mpl.colors.Normalize(vmin=0,vmax=1)
                    
                    ax.contourf(np.radians(Thetanoise_grid),Rnoise_grid,LRnoise_interp,cmap=cmapnoise,norm=normnoise,levels=30)
                    ...
                    if saveFig==True:
                        plt.savefig(f'{os.path.join(self.log_dir, filename)}/Landing_Rate_Fig_20Trials_Noise.pdf',dpi=300)
        ...
        if saveFig==True:
            plt.savefig(f'{os.path.join(self.log_dir, filename)}/BigPlotNoise.pdf',dpi=300)
        ...
        if saveFig==True:
            plt.savefig(f'{os.path.join(self.log_dir, filename)}/AvgPlotNoise.pdf',dpi=300)
        ...
        if saveFig==True:
            plt.savefig(f'{os.path.join(self.log_dir, filename)}/Avg4LegBCPlotNoise.pdf',dpi=300)
        ...
        if saveFig==True:
            plt.savefig(f'{os.path.join(self.log_dir, filename)}/Avg2LegPlotNoise.pdf',dpi=300)
        ...
        if saveFig==True:
            plt.savefig(f'{os.path.join(self.log_dir, filename)}/AvgNoLegPlotNoise.pdf',dpi=300)
