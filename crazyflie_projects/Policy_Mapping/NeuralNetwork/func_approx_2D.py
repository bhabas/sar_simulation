...
class Model(nn.Module):
    def __init__(self,in_features=2,h1=10,h2=10,out_features=2):
        super().__init__()
...
def train_model(epochs,X_train,y_train):
    ...
    torch.save(model,'Func_approx_2D.pt')
    return model
...
if __name__ == '__main__':
    ...
    data_array = np.stack((X1.flatten(),X2.flatten(),y1.flatten(),y2.flatten())).T
    df = pd.DataFrame(data_array,columns=['X1','X2','y1','y2'])

    ## SPLIT DATA FEATURES INTO TRAINING AND TESTING DATA
    train, test = train_test_split(df,test_size=0.2,random_state=33)

    ## CONVERT DATA INTO TENSORS
    X_train = torch.FloatTensor(train[['X1','X2']].to_numpy())
    X_test = torch.FloatTensor(test[['X1','X2']].to_numpy())
