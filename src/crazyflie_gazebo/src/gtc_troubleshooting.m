...
%% Current State
pos = [0.2517 0.1439 0.4880]';
vel = [0.0054,0.0022,-0.0001]';
omega = [-0.0054 ,-0.0068,-0.0014]';

eul = [0,5*pi/180,0]; 
R = eul2rotm(eul,'ZYX');

R = [0.9915 -.1298 0.0007;
    .1298 0.9915 -0.0010;
    -0.0005 0.0011 0.9999]
...
e_x = pos-x_d;
e_v = vel-v_d;
A = -kp_x*e_x-kd_x*e_v + m*g*e3 + m*a_d;
u1 = dot(A,R*e3);
...
u2 = -kp_R*e_R - kd_R*e_omega + cross(omega,J*omega) - J*(hat(omega)*R'*R_d*omega_d - R'*R_d*domega_d);
