...
class MavrosTestCommon(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.sub_topics_ready = {
            ...
            for key in [
                ...
                'local_vel','mission_wp', 'state', 'real_pose'
            ]
        }
        ...
        self.state_sub = rospy.Subscriber('mavros/state', State,
                                          self.state_callback)
        self.real_pose_sub = rospy.Subscriber('ground_truth/state',
                                              Odometry,
    ...
    def real_pose_callback(self, data):
        ...
        if not self.sub_topics_ready['real_pose']:
            self.sub_topics_ready['real_pose'] = True
    ...
    def set_mode(self, mode, timeout):
        ...
        for i in xrange(timeout * loop_freq):
            if self.state.mode == mode:
                mode_set = True
                rospy.loginfo("set mode: {0} success | seconds: {1} of {2}".format(mode,
                    i / loop_freq, timeout))
                break
            else:
                try:
