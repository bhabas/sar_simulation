...
def runGraph():
    ...
    ax2.grid(True)
    ax2.set_title(f'Rollouts: {DashNode.n_rollouts*2} | Episode: {DashNode.k_ep} Run: {DashNode.k_run} (Completed) ')
    fig_1.tight_layout()


    ## INIT EMPTY ARRAYS TO BE FILLED
    k_ep_arr = [0] # Need zero value just to be able to call [-1]
    k_run_arr = [0] 
    reward_arr = [0]
    ...
    def animate_reward(i,k_ep_arr,k_run_arr,reward_arr):
        
        if DashNode.k_run != k_run_arr[-1]:
