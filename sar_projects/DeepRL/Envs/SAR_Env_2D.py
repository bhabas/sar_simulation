...
class SAR_Env_2D(gym.Env):
    ...
    def __init__(self,GZ_Timeout=True,My_range=[-8.0e-3,8.0e-3],Vel_range=[1.5,3.5],Phi_rel_range=[0,90],Plane_Angle_range=[90,180]):
        ...
        gamma = np.deg2rad(10)  # Leg Angle [m]
    ...
    def reset(self, seed=None, options=None, V_mag=None, Phi_rel=None):
        ...
        self._set_state(r_BO[0],r_BO[1],np.radians(-135),V_BO[0],V_BO[1],0)
            ...
            ## IMPACT TRUE
                ...
                ## LEG 1 CONTACT
                    ...
                    while not self.Done:
                        ...
                        beta,dbeta = self._iter_step_Swing(beta,dbeta)
    ...
    def _beta_prop(self,impact_leg):
        ...
        if impact_leg == 1:
            ...
            a = np.sqrt(PD**2 + L**2 - 2*PD*L*np.cos(np.pi/2-gamma))
            beta_max = np.pi - np.arccos((L**2 + a**2 - PD**2)/(2*a*L))
    ...
    def _impact_conversion(self,impact_state,params,impact_conditions):
        ...
        if Leg1Contact:
            ...
            ## CALC BETA ANGLE
            Beta = np.pi/2 - phi - self.Plane_Angle_rad - gamma
            Beta = Beta % np.pi
...
if __name__ == '__main__':
    env = SAR_Env_2D(Vel_range=[1.0,1.0],Phi_rel_range=[45,45],Plane_Angle_range=[180,180])
    env.RENDER = True
